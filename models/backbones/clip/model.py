...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x, return_feature=False):
        ...
        x3 = self.layer3(x2)
        x4 = self.layer4(x3)
        x_pool = self.attnpool(x4)

        if return_feature:
            return x_pool, [x1, x2, x3, x4]
        else:
            return x_pool
...
class Transformer(nn.Module):
    ...
    def forward_attn_layers(self, x):
        features = []
    ...
    def forward(self, x: torch.Tensor, return_feature=False): # x: LND
        x, fetaures = self.forward_attn_layers(x)
        if return_feature:
            return x, fetaures
        else:
            return x
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_feature=False):
        ...
        if return_feature:
            x, features = self.transformer(x, return_feature)
...
if __name__ == '__main__':
    ...
    attn_pool = AttentionPool2d(56, 256, 32, 256)
    y = attn_pool(x)
    print(y.size())
