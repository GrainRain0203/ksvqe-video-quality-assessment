...
def load_clip_to_cpu(backbone_name):
    ...
    local_name = backbone_name.replace("/", "-") + ".pt"
    local_path = Path(
        os.environ.get(
            "KSVQE_CLIP_WEIGHTS",
            repo_root / "pretrained_weights" / "clip" / local_name,
        )
    )
    model_path = str(local_path) if local_path.is_file() else clip._download(url)
    try:
        # loading JIT archive
